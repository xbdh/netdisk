...
func multipartUpload(filename string, targetURL string, chunkSize int) error {
	...
	for {
		...
		bufCopied := make([]byte, 10*1048576)
		copy(bufCopied, buf)
		...
		go func(b []byte, curIdx int) {
			...
			resp, err := http.Post(
				targetURL+"chunkid="+strconv.Itoa(curIdx),
				"multipart/form-data",
				bytes.NewReader(b))
			if err != nil {
				fmt.Println(err)
...
func main() {
	...
	// 3. 请求分块上传接口
	filename := "/home/chen/download/docker-ce_18.06.3_ce_3-0_ubuntu_amd64.deb"
	tURL := "http://localhost:8081/file/mp/upload?" + "fileid=41960292078321665&"
	multipartUpload(filename, tURL, 10*1024*1024)
